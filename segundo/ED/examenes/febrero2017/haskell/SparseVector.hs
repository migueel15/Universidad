...
vector n x
  | n < 0 = error "size del vector negativo"
...
simplify l@(Unif x) r@(Node ln rn) = Node l r
simplify l@(Node ln rn) r@(Unif x) = Node l r
simplify l@(Node ln rn) r@(Node ln2 rn2) = Node l r

-------------------------------------------------------------------------------
-- | Exercise d.
get:: Int -> Vector a -> a
get idx (V tam (Unif x))
  | idx < 0 || idx > (tam-1) = error "indice incorrecto"
  | otherwise = x
get idx (V tam (Node ln rn))
  | idx < 0 || idx > tam-1 = error "indice incorrecto"
  | otherwise = getIdx' idx tam (Node ln rn)

getIdx':: Int -> Int -> Tree a -> a
getIdx' idx tam (Unif x) = x
getIdx' idx tam (Node ln rn)
  | idx <= (div tam 2 - 1) = getIdx' idx (div tam 2 ) ln
  | otherwise = getIdx' (idx - div tam 2) (div tam 2) rn

-------------------------------------------------------------------------------
-- | Exercise e.

set:: (Eq a) => Int -> a -> Vector a -> Vector a
set idx val v@(V tam tree)
  | idx < 0 || idx > tam-1 = error "indice fuera de rango"
  | otherwise = V tam (set' idx val tam tree)

set':: (Eq a) => Int -> a -> Int -> Tree a -> Tree a
set' idx val 1 (Unif x) = Unif val
set' idx val tam (Unif x) = set' idx val tam (Node (Unif x) (Unif x))
set' idx val tam (Node ln rn)
  | idx <= (div tam 2 - 1) = simplify (set' idx val (div tam 2) ln) rn
  | otherwise = simplify ln (set' (idx - div tam 2) val (div tam 2) rn)
...
mapVector fun (V tam tree) = V tam (mapVector' fun tree)

mapVector':: (Eq a, Eq b) => (a -> b) -> Tree a -> Tree b
mapVector' fun (Unif x) = Unif (fun x)
mapVector' fun (Node ln rn) = simplify (mapVector' fun ln) (mapVector' fun rn)
