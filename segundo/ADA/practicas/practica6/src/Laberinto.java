...
public class Laberinto {
  ...
  private boolean encontrarCamino(List<Posicion> sol) {
    if (esCompleta(sol)) {
      ...
      ArrayList<Posicion> candidatos = posicionesCandidatas(sol.get(sol.size() - 1), sol);
  ...
  public List<Posicion> encontrarCaminoMasCortoBB() {
    ...
    while (!c.estaVacia()) {
      ...
      if (esCompleta(act.getCamino())) {
        ...
        List<Posicion> hijosCandidatos = posicionesCandidatas(act.getCamino().get(act.getCamino().size() - 1),
            act.getCamino());
