...
class BilliardTable:
    ...
    def draw(self, screen):
        ...
        pygame.draw.rect(
            screen,
            color_madera,
            (0, OFFSET_Y - 15, SCREEN_WIDTH, SCREEN_HEIGHT - OFFSET_Y + 15),
        )
        ...
        pygame.draw.circle(
            screen, (0, 0, 0), to_pygame((head_spot_x, y_center)), spot_radius
        )

        # Punto de pie (Foot Spot) - centro del triángulo
        foot_spot_x = self.margin + (3 * tapete_w_m / 4)
        pygame.draw.circle(
            screen, (0, 0, 0), to_pygame((foot_spot_x, y_center)), spot_radius
        )
...
class Ball:
    def __init__(self, space, pos, color, number=None, is_cue=False, punto=False):
        ...
        self.font = pygame.font.SysFont(
            "Arial", int(self.radius * PX_M * 0.8), bold=True
        )
    ...
    def apply_advanced_friction(self, dt):
        ...
        self.body.apply_force_at_world_point(
            -v.normalized() * f_final, self.body.position
        )
    ...
    def draw(self, screen):
        ...
        if self.punto:
            pygame.draw.circle(
                screen,
                (5, 5, 5),
                (pos_px[0] - rad_px // 3, pos_px[1] - rad_px // 3),
                rad_px // 5,
                width=0,
            )
...
class Ball_striped(Ball):
    ...
    def draw(self, screen):
        ...
        pygame.draw.line(
            temp_surf, self.stripe_color, (p1_x, p1_y), (p2_x, p2_y), int(rad_px * 1.1)
        )
...
def setup_game(space, mesa):
    ...
    # Esta es la blanca
    bolas.append(
        Ball(space, (head_spot_x, y_center), (255, 255, 255), is_cue=True, punto=True)
    )
...
def draw_aiming_help(screen, ball, cue_angle):
    ...
    end_p1 = (
        start_p1[0] + direction.x * line_len_px,
        start_p1[1] + direction.y * line_len_px,
    )
    end_p2 = (
        start_p2[0] + direction.x * line_len_px,
        start_p2[1] + direction.y * line_len_px,
    )
...
def main():
    ...
    while running:
        ...
        for i, b in enumerate(lisas_coladas):
            ...
            pygame.draw.circle(
                screen, (255, 255, 255), (pos_ui[0] - 5, pos_ui[1] - 5), 4
            )
        ...
        if not blanca.in_pocket and blanca.body.velocity.length < 0.02:
            ...
            power_text = font_ui.render(
                f"IMPULSO: {cue_power:.1f}", True, (255, 255, 255)
            )
            ...
            pygame.draw.rect(
                screen, (255, 0, 0), (30, 60, (cue_power / MAX_POWER) * bar_width, 10)
            )
