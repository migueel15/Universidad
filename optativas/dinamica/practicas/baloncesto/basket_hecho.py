...
class Tsim:
	...
	def pone_fondo(self,imagen):
		try:
			fondo = pygame.image.load(self.ruta_recurso(imagen)).convert()
...
class Tbalon(Tobjeto):
	def __init__(self, sim, pos_m, masa_kg=0.625, radio_m=0.119, img_path="balon_basket.png", Cd=0.54, k_magnus=0.7):
	...
	def _preparar_imagen(self, path):
		...
		try:
			...
			img = pygame.image.load(self.sim.ruta_recurso(path)).convert_alpha()  #convert_alpha para los png transparentes
	...
	def draw(self):
		...
		angulo_deg = math.degrees(-self.body.angle) % 360
		...
		if not math.isfinite(pos.x) or not math.isfinite(pos.y):
			self.sujetar_en_mano(self.pos_mano)
			pos = self.body.position
	...
	def aplicar_rozamiento_aire(self, v_viento=(0,0)):
		"""Arrastre cuadratico sencillo para un balon de baloncesto."""
		v_rel = self.body.velocity * self.sim.M_PX - Vec2d(*v_viento)
		v_rel_mag = v_rel.length
		if v_rel_mag < 0.1:
			return
		f_drag = -0.5 * self.rho_aire * self.Cd * self.area_m2 * v_rel_mag * v_rel / self.sim.M_PX
		self.body.apply_force_at_world_point(f_drag, self.body.position)
	#---------------------------------------------------------------
	def aplicar_magnus(self, v_viento=(0,0)):
		"""Fuerza Magnus simplificada: suficiente para ver el efecto del spin."""
		v_rel = self.body.velocity * self.sim.M_PX - Vec2d(*v_viento)
		v_rel_mag = v_rel.length
		if v_rel_mag < 0.1 or abs(self.body.angular_velocity) < 0.01:
			return
		R = self.radio_m
		S = R * self.body.angular_velocity / v_rel_mag
		Cm = self.k_magnus * S / (2 + abs(S))
		u_magnus = Vec2d(-v_rel.y, v_rel.x).normalized()
		f_magnus = 0.5 * self.rho_aire * Cm * self.area_m2 * v_rel_mag**2 * u_magnus / self.sim.M_PX
	...
	def aplicar_frenado_rotacional(self):
		...
		if abs(w) < 0.01:
			return
		torque_m = -0.5 * self.rho_aire * self.Cm_rotacional * abs(w) * w * self.radio_m**5
...
class Ttablero(Tobjeto):
	def __init__(self, sim, x_tablero_m=7, color=(100, 100, 100)):
		...
		for aro_shape in [self.aro_trasero_shape, self.aro_delantero_shape, self.aro_sop_shape]:
	...
	def _crear_red(self):
		...
		for anclaje in [self.body_delantero, self.body_trasero]:
			...
			for dist in distancias:
				...
				nodo.position = (cuerpo_padre.position.x, cuerpo_padre.position.y + dist)
	...
	def _dibujar_red(self):
		...
		for cadena in self.cadenas_red:
			...
			for nodo in cadena['nodos']:
				pos = nodo.position
				pygame.draw.line(self.sim.screen, color_red, (int(padre_pos.x), int(padre_pos.y)), (int(pos.x), int(pos.y)), 1)
				padre_pos = pos
		...
		for i in range(len(bolas_izq)):
			pygame.draw.line(self.sim.screen, color_red, (int(bolas_izq[i].position.x), int(bolas_izq[i].position.y)), (int(bolas_der[i].position.x), int(bolas_der[i].position.y)), 1)
			if i > 0:
				pygame.draw.line(self.sim.screen, color_red, (int(bolas_izq[i-1].position.x), int(bolas_izq[i-1].position.y)), (int(bolas_der[i].position.x), int(bolas_der[i].position.y)), 1)
				pygame.draw.line(self.sim.screen, color_red, (int(bolas_der[i-1].position.x), int(bolas_der[i-1].position.y)), (int(bolas_izq[i].position.x), int(bolas_izq[i].position.y)), 1)

		for nodo in bolas_izq + bolas_der:
			pygame.draw.circle(self.sim.screen, color_red, (int(nodo.position.x), int(nodo.position.y)), max(1, int(self._distancia_m_a_px(0.012))))
	#---------------------------------------------------------------
	def draw(self):
...
class Tjugador(Tobjeto):
	def __init__(self, sim, pos_m=(2,0), imagenes=("jugador01.png", "jugador02.png"), alturas_m=(2.0, 2.2)):
		super().__init__(sim)
		self.pos_m = pos_m
		self.lanzando = False
		self.imagenes = []
	...
	def _preparar_imagen(self, path, altura_m):
		...
		try:
			img = pygame.image.load(self.sim.ruta_recurso(path)).convert_alpha()
			...
			pygame.draw.circle(surf, (230, 190, 130), (p_width//2, int(0.12*p_height)), int(0.08*p_height))
			pygame.draw.rect(surf, (50, 90, 180), (int(0.30*p_width), int(0.20*p_height), int(0.40*p_width), int(0.45*p_height)))
			pygame.draw.line(surf, (30, 30, 30), (p_width//2, int(0.65*p_height)), (int(0.20*p_width), p_height), 3)
			pygame.draw.line(surf, (30, 30, 30), (p_width//2, int(0.65*p_height)), (int(0.80*p_width), p_height), 3)
			return surf
	#---------------------------------------------------------------
	def draw(self):
...
if __name__ == "__main__":
	bk=Tbasket(x_tablero_m=7,pos_balon_m=(2,2),PX_M=120,width=1000,height=600,suelo=600,gravedad=(0,-9.81),fondo='grada_baloncesto03.jpg')
