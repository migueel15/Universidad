...
try:
	from arrastre_reynolds import get_Cd, mach_correction
except ImportError:
...
class BallisticsUI:
	def __init__(self, root):
		...
		self.root.geometry("550x750") # Aumentado ligeramente para los checkbox
	...
	def setup_ui(self):
		...
		# Panel de Masa
		info_frame = ttk.LabelFrame(main_frame, text=" Estado del Proyectil ", padding="10")
	...
	while running:
		...
		if not sim_frozen:
			for _ in range(sub_steps):
				...
				force = -0.5 * cd_calc * RHO_AIR * (math.pi * radius**2) * (speed**2) * body.velocity.normalized()
