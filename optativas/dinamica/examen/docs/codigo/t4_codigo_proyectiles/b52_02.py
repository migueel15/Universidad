...
from rozamiento_aire import mach_correction, vel_sonido_temp, vel_sonido_altitud, get_rho
...
######################################################################################
############# AQUÍ ESTÁ LA FÍSICA ####################################################
############# Se usan las funciones definidas en rozamiento_aire.py
######################################################################################
def aplicar_newton(body, CONF, AREA_M2, M_PX):
	if not CONF["arrastre"]: return #si no consideramos arrastre nos vamos
	alt_m = (550 - body.position.y) * M_PX
	#calcula la densidad en función de la altitud
	#si no está activado se toma el valor por defecto 1.225
	rho = get_rho(alt_m) if CONF["densidad"] else 1.225
	v_ms = body.velocity * M_PX
	speed = v_ms.length
	if speed < 0.1: return  #evita que mueva hacia atrás
	cd = CONF["cd"]  # Para no liarla tomamos un único valor de Cd (del dialogbox) en lugar
					 #de usara la función get_Cd definida en rozamiento_aire.py
	if CONF["mach"]:   #si está activado, aplicamos la corrección de mach
		# tenemos en cuenta que la velocidad del sonido depende de la altitud
		cd *= mach_correction(speed,vel_sonido_altitud(alt_m))
	
	#Fuerza de drag según el modelo de Newton
	f_drag_total = - 0.5 * rho * cd * AREA_M2 * speed * v_ms
	print(cd,rho,f_drag_total/M_PX)
	body.apply_force_at_world_point((f_drag_total) / M_PX, body.position)
######################################################################################
######################################################################################	
...
def run_simulation():
	...
	while running:
		...
		for b in bombas_aire: aplicar_newton(b["body"], CONF, AREA_M2, M_PX)
		space.step(dt)
		t_sim += dt
