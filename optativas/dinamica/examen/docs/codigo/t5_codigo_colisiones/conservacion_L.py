...
Para evitar los errores acumulados por el solver que usa Euler, hacemos un
SUBSTEPPING: En cada paso de reloj (1/60 s) hacemos un número 
de pasos de simulación igual a 'pasos_fisica'. La pantalla sólo se refresca
60 veces por segundo pero el dt=1/(60*pasos_fisica) con lo que el cálculo
es más preciso'''
...
def escribir(screen, texto, x, y, color=(0, 0, 0)):
...
def simular():
	...
	while running:
		...
		######################################################
		#### TODO ESTO SE METE EN UN BUCLE Y SE REPITE	
		#### EL NUMERO DE VECES QUE QUERAMOS POR CADA FRAME
		####
		######################################################
		...
		# Momento lineal	
		P=masa*body.velocity
		# Calculo el momento angular como producto vectorial
		# de el vector de posición del objeto (-dx,-dy) por
		# el momento lineal	
		L=-dx*P.y+dy*P.x

		escribir(screen, f'Momento Lineal:  ({P.x:.2f},{P.y:.2f})', 10, 20)
		escribir(screen, f'Módulo:          {P.length:.2f}', 10, 40)
		escribir(screen, f'Momento Angular: {L/100:.2f}', 10, 70)
		...
		### Este lo hemos quitado porque el space.step se hace
		### dentro del bucle
		### space.step(1/60.0)
		pygame.display.flip()
		clock.tick(60)
