...
def inicia_juego():
	...
	def begin_taco_bola(arbiter, space, data):
		# 1. Identificar formas
		s_taco, s_bola = arbiter.shapes
		# Asegurarnos de quién es quién por el collision_type
		if s_taco.collision_type != 2: 
			s_taco, s_bola = s_bola, s_taco
		
		taco = s_taco.parent
		bola = s_bola.parent
	
		# 2. CALCULAR IMPULSO PERSONALIZADO
		# Por ejemplo, transferimos el 90% de la energía lineal del taco
		v_impacto = taco.velocity
		impulso_lineal = v_impacto * taco.mass * 0.2
		
		# Calculamos un impulso angular basado en el "offset" del choque
		# (Si el taco no apunta al centro de la bola)
		punto_choque = arbiter.contact_point_set.points[0].point_a
		offset = punto_choque - bola.position
		# Simplificación: el torque depende de la perpendicularidad del golpe
		impulso_angular = offset.cross(impulso_lineal) * 2.0 
	
		# 3. APLICAR CAMBIOS MANUALMENTE
		bola.apply_impulse_at_world_point(impulso_lineal, punto_choque)
		bola.angular_velocity += impulso_angular / bola.moment
		
		# 4. PARAR EL TACO
		taco.velocity = (0, 0)
		taco.angular_velocity = 0
	
		# 5. ¡ESTA ES LA CLAVE!
		# Devolvemos False para que Pymunk NO aplique su propia física de choque
		return False
	
	# En tu función inicia_espacio o configuración:
	space.on_collision(2, 1, begin=begin_taco_bola)
