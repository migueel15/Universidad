...
class ImageGenerator:
    ...
    def generate_channel_with_entropy(
        self, target_entropy: float, seed: Optional[int] = None
    ) -> np.ndarray:
        """
        ...
        if target_entropy == 0.0:
            ...
            channel = np.full((self.height, self.width), 255, dtype=np.uint8)
    ...
    def generate_image(
        ...
        channel_g = self.generate_channel_with_entropy(
            entropy_g, seed + 1 if seed else None
        )
        channel_b = self.generate_channel_with_entropy(
            entropy_b, seed + 2 if seed else None
        )
    ...
    def load_raw(self, filename: str) -> Image.Image:
        ...
        if not os.path.exists(metadata_filename):
            raise FileNotFoundError(
                f"No se encontró el archivo de metadata: {metadata_filename}"
            )
        ...
        print(
            f"  Entropías: R={metadata['entropy_R']:.2f}, G={metadata['entropy_G']:.2f}, B={metadata['entropy_B']:.2f}"
        )

        return image

    def load_image_from_file(
        self, filename: str, resize_to: Optional[Tuple[int, int]] = None
    ) -> Image.Image:
        """
        ...
        print(
            f"  Entropías: R={stats['R']['entropy']:.2f}, G={stats['G']['entropy']:.2f}, B={stats['B']['entropy']:.2f}"
        )
