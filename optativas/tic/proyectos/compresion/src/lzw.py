...
class LZWEncoder:
    ...
    def bytes_to_codes(
        self, compressed_bytes: bytes, bits_per_code: int, padding: int, num_codes: int
    ) -> List[int]:
        """
    ...
    def decode_image(self, compressed_bytes: bytes, metadata: dict) -> Image.Image:
        ...
        if len(decoded_bytes) != metadata["original_size"]:
            print(
                f"⚠ Advertencia: tamaño decodificado ({len(decoded_bytes)}) != original ({metadata['original_size']})"
            )

        # Reconstruir imagen
        img_array = np.frombuffer(
            decoded_bytes[: metadata["original_size"]], dtype=np.uint8
        )
    ...
    def save_compressed(self, compressed_bytes: bytes, metadata: dict, filename: str):
        ...
        print(
            f"✓ Comprimido guardado: {filename}.lzw ({len(compressed_bytes):,} bytes)"
        )
...
def test_lzw():
    ...
    print(
        f"Tamaño diccionario usado: {min(metadata['num_codes'] + 256, encoder.max_dict_size)}"
    )
