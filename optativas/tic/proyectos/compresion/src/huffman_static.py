...
class HuffmanNode:
    ...
    def __init__(
        self, symbol: Optional[int] = None, frequency: int = 0, left=None, right=None
    ):
...
class HuffmanStaticEncoder:
    ...
    def generate_codes(
        self, root: HuffmanNode, code: str = "", codes: Optional[Dict[int, str]] = None
    ) -> Dict[int, str]:
        """
    ...
    def encode_image(self, image: Image.Image) -> Tuple[bytes, dict]:
        ...
        # Convertir a bytes
        compressed_bytes = int(all_bits, 2).to_bytes(
            len(all_bits) // 8, byteorder="big"
        )
    ...
    def decode_channel(
        self, bits: str, tree: HuffmanNode, num_pixels: int
    ) -> np.ndarray:
        """
    ...
    def save_compressed(self, compressed_bytes: bytes, metadata: dict, filename: str):
        ...
        print(
            f"✓ Comprimido guardado: {filename}.huf ({len(compressed_bytes):,} bytes)"
        )
...
def test_huffman():
    ...
    for channel in ["R", "G", "B"]:
        ...
        print(
            f"  Canal {channel}: {len(codes)} símbolos únicos, long. media código: {avg_len:.2f} bits"
        )
    ...
    print(
        f"Verificación: {'✓ CORRECTO - Descompresión perfecta' if son_iguales else '✗ ERROR'}"
    )
